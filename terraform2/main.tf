...
# 1. First enable the SQL Admin API explicitly
resource "google_project_service" "sqladmin_api" {
  service            = "sqladmin.googleapis.com"
  disable_on_destroy = false
}

# 2. Then enable other required services
resource "google_project_service" "required_services" {
  ...
  # Don't create until SQL Admin API is ready
  depends_on = [google_project_service.sqladmin_api]
}

# 3. IAM permissions (wait until APIs are enabled)
...
resource "google_sql_database_instance" "postgres_instance" {
  ...
  depends_on = [
    ...
    google_project_iam_member.terraform_sql_admin
    ]
}
